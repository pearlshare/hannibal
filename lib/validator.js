...
function Hannibal (opts) {
  ...
  if (opts) {
    if (opts.pre) {
      pre = lodash.defaultsDeep({}, defaultPre, opts.pre);
    }
    if (opts.validators) {
      validators = lodash.defaultsDeep({}, defaultValidators, opts.validators);
    }
  }
...
Hannibal.prototype.fetchValidator = function fetchValidator (type, name) {
  if (this.validators[type] && this.validators[type][name] && typeof this.validators[type][name] === "function") {
  ...
  function validate (testSchema, value, output, error, validatorObj) {
    ...
    if (testSchema.pre) {
      [].concat(testSchema.pre).forEach(function (f) {
        if (typeof f === "string") {
          try {
            value = self.pre[f](value);
          } catch (e) {
            throw new Error("no pre function registered called: " + f);
          }
        } else if (typeof f === "function") {
          try {
            value = f(value);
          } catch (e) {
            throw new Error("pre function '" + f + "' threw error: " + e);
          }
        }
      });
    }
      ...
      if (testSchema.validators) {
        ...
        lodash.forIn(testSchema.validators, function (validator, validatorName) {
          if (validatorName === "custom" && typeof validator === "function") {
            ...
            try {
              self.fetchValidator(type, validatorName)(validator, value);
