...
module.exports = function validate (testSchema, value, output, error, validatorObj) {
  ...
  // Recurse into any objects and build the validated output
    ...
    output = {};
    Object.keys(testSchema.schema).forEach(function (key) {
      var rslt = validate(
  ...
  if (output !== undefined) {
    ...
    if (testSchema.validators) {
      ...
      Object.keys(testSchema.validators).forEach(function(validatorName) {
        var validator = testSchema.validators[validatorName];
