 ...
 */
function findNamedSchemas (schema, found) {

  if (schema.id) {
    if (found[schema.id]) throw new Error("Schema id already defined: " + testSchema.id);
  ...
  if (schema.type === "array") {
    if (schema.schema) {
      findNamedSchemas(schema.schema, found);
    }

  } else if (schema.type === "object") {
    if (schema.schema) {
      Object.keys(schema.schema).forEach(function (key) {
        findNamedSchemas(schema.schema[key], found);
      });
    }
  }
...
Hannibal.prototype.create = function buildValidator (schema) {
  ...
  var namedSchemas = findNamedSchemas(schema, {});
